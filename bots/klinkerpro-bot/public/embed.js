(function () {
  ...
  function mount() {
    ...
    var MANAGER_PHONE_LABEL = '+7 (921) 745-77-55';
    var MANAGER_PHONE_TEL = '+79217457755';
    var MAX_MESSENGER_URL =
      'https://max.ru/u/f9LHodD0cOIpC25EaD4tYfNKs2IixN6hR4GPJ7lLCZmLWlK0NXznAW7oYyM';
    /** Маскот в шапке диалога на мобиле (×2 от десктопных 40px) */
    var MOBILE_HEAD_MASCOT_PX = 80;
    ...
    var HEAD_ASSIST_LABEL_COPY =
      '<span class="kpb-btn-copy kpb-head-copy">' +
      '<span class="kpb-btn-line1">AI помощник</span>' +
      '<span class="kpb-btn-line2 kpb-head-line2">' +
      '<span class="kpb-head-l2a">знаю ответы,</span>' +
      '<span class="kpb-head-l2b">сделаю расчёт</span>' +
      '</span></span>';
    var BTN_LABEL =
      '<span class="kpb-btn-icon">' + ICON_HOME + '</span>' + ASSIST_LABEL_COPY;
    function mobileFabBtnLabel(mascotSrc) {
      return (
        '<span class="kpb-btn-icon">' +
        '<img class="kpb-btn-mascot" src="' +
        (mascotSrc || resolveMascotUrl('')) +
        '" width="' +
        MOBILE_FAB_MASCOT_PX +
        '" height="' +
        MOBILE_FAB_MASCOT_PX +
    ...
    function headAssistLabel(mascotSrc) {
      return (
        '<div class="kpb-head-assist" aria-hidden="false">' +
        '<span class="kpb-btn-icon">' +
        '<img class="kpb-head-mascot" src="' +
        (mascotSrc || resolveMascotUrl('')) +
        '" alt="" aria-hidden="true" decoding="async" />' +
        '</span>' +
        '<div class="kpb-head-assist-main">' +
        HEAD_ASSIST_LABEL_COPY +
        '<a class="kpb-head-phone kpb-tel" href="tel:' +
        MANAGER_PHONE_TEL +
        '">' +
        MANAGER_PHONE_LABEL +
        '</a>' +
        '<a class="kpb-head-max kpb-link" href="' +
        MAX_MESSENGER_URL +
    ...
    var FAB_MASCOT_HEIGHT_PX = 100;
    /** Иконка маскота в мобильной кнопке FAB (слот .kpb-btn-icon) */
    var MOBILE_FAB_MASCOT_PX = 40;
    var MOBILE_FAB_BOTTOM_PX = 40;
    var REVEAL_LINE_MS = 90;
    ...
    function endAutoIntro() {
      ...
      markIntroSeen();
      fabShell().classList.remove('kpb-scroll-hidden');
    }
    ...
    style.textContent = [
      ...
      '#kpb-mascot{height:100px;width:auto;display:block;margin:0;padding:0;border:0;pointer-events:auto;cursor:pointer;line-height:0;vertical-align:bottom;-webkit-tap-highlight-color:transparent;opacity:1}',
      ...
      '@media (max-width:1024px){#kpb-fab-stack{display:none !important}#kpb-btn{position:fixed !important;z-index:99990 !important}#kpb-btn .kpb-btn-icon{width:40px;height:40px}#kpb-btn.kpb-hidden,#kpb-btn.kpb-scroll-hidden{display:none !important;visibility:hidden !important;opacity:0 !important;pointer-events:none !important}}',
      ...
      '#kpb-btn .kpb-btn-icon{flex-shrink:0;display:flex;align-items:center;justify-content:center;width:34px;height:34px}',
      '#kpb-head .kpb-btn-icon{flex-shrink:0;display:flex;align-items:center;justify-content:center;width:40px;height:40px}',
      '#kpb-btn .kpb-btn-home,#kpb-head .kpb-btn-home{display:block;width:34px;height:34px}',
      '#kpb-head .kpb-head-mascot{display:block;width:40px;height:40px;object-fit:contain;object-position:center}',
      '#kpb-btn .kpb-btn-mascot{display:block;width:40px;height:40px;object-fit:contain;object-position:center}',
      ...
      '#kpb-head .kpb-logo-square{display:none;height:36px;width:auto;max-width:120px;max-height:36px;object-fit:contain}',
      ...
      '#kpb-head .kpb-head-line2{display:block;white-space:nowrap}',
      '#kpb-head .kpb-head-l2a,#kpb-head .kpb-head-l2b{display:inline}',
      '#kpb-head .kpb-head-l2b::before{content:" "}',
      '#kpb-head .kpb-head-phone{display:none;flex-shrink:0;pointer-events:auto;margin-left:60px}',
      '#kpb-head .kpb-head-max{display:none;flex-shrink:0;pointer-events:auto;margin-left:10px;font-weight:700;color:#fff !important;text-decoration:none !important;background:#c04500;border-radius:30px;padding:8px 14px;line-height:1.2;box-shadow:0 2px 6px rgba(0,0,0,.12)}',
      '#kpb-root #kpb-head a.kpb-head-max.kpb-link:hover{background:#a83800;color:#fff !important}',
      '#kpb-root #kpb-head a.kpb-head-phone.kpb-tel{text-decoration:none !important;color:#fff !important;font-weight:700;background:#c04500;border-radius:30px;padding:8px 14px;line-height:1.2;box-shadow:0 2px 6px rgba(0,0,0,.12)}',
      '#kpb-root #kpb-head a.kpb-head-phone.kpb-tel:hover{background:#a83800;color:#fff !important}',
      ...
      '@media (max-width:1024px){#kpb-panel{width:90vw;max-width:90vw;height:86.25vh;max-height:86.25vh;top:auto;bottom:0;border-radius:5px 0 0 0}#kpb-panel.kpb-compact{height:43.125vh;max-height:43.125vh}#kpb-panel.kpb-compact.kpb-intro{width:100vw !important;max-width:100vw !important;height:60.375vh;max-height:60.375vh;bottom:80px}#kpb-head{height:88px;min-height:88px;max-height:88px;padding:0 40px 0 10px;gap:12px}#kpb-head .kpb-logo-wide,#kpb-head .kpb-logo-square,#kpb-panel.kpb-intro #kpb-head .kpb-logo-square{display:none !important}#kpb-head .kpb-head-assist{margin-left:0;gap:14px;flex:1 1 auto;width:100%;min-width:280px;max-width:100%}#kpb-panel.kpb-intro #kpb-head .kpb-head-assist{margin-left:0}#kpb-head .kpb-head-assist .kpb-btn-icon{width:80px;height:80px;flex-shrink:0;margin-left:0 !important}#kpb-head .kpb-head-mascot{width:80px;height:80px}#kpb-head .kpb-head-phone{display:none !important}#kpb-head .kpb-head-assist .kpb-btn-icon{margin-left:0}#kpb-btn .kpb-btn-icon{margin-left:5px}#kpb-head .kpb-head-assist-main .kpb-head-copy{margin-left:10px;padding-left:0}#kpb-head .kpb-btn-line1{font-size:23px;line-height:1.05}#kpb-head .kpb-btn-line2{font-size:16px;line-height:1.08;margin-top:1px;white-space:normal}#kpb-head .kpb-head-line2{display:flex;flex-direction:column;align-items:flex-start}#kpb-head .kpb-head-l2a,#kpb-head .kpb-head-l2b{display:block;font-size:inherit;line-height:inherit}#kpb-head .kpb-head-l2b::before{content:none}#kpb-close .kpb-ico{width:22px;height:22px}#kpb-btn{position:fixed !important;z-index:99990 !important;left:20px !important;right:auto !important;bottom:40px !important;width:220px !important;min-width:220px !important;max-width:220px !important;justify-content:flex-start !important;background:rgba(213,77,0,.8) !important;border:1px solid rgba(255,255,255,.8) !important}#kpb-btn:hover{background:rgba(192,69,0,.8) !important}}',
      '@media (max-width:480px){#kpb-panel{height:80.5vh;max-height:80.5vh}#kpb-panel.kpb-compact{height:40.25vh;max-height:40.25vh}#kpb-panel.kpb-compact.kpb-intro{width:100vw !important;max-width:100vw !important;height:56.35vh;max-height:56.35vh;bottom:80px}#kpb-head{height:88px;min-height:88px;max-height:88px;padding:0 36px 0 8px;gap:12px}#kpb-head .kpb-logo-square{display:none !important}#kpb-head .kpb-head-assist{margin-left:0;min-width:260px}#kpb-panel.kpb-intro #kpb-head .kpb-head-assist{margin-left:0}#kpb-head .kpb-head-assist .kpb-btn-icon{width:80px;height:80px;margin-left:0 !important}#kpb-head .kpb-head-mascot{width:80px;height:80px}#kpb-btn .kpb-btn-icon{margin-left:5px}#kpb-head .kpb-head-assist-main .kpb-head-copy{margin-left:10px}#kpb-head .kpb-btn-line1{font-size:21px}#kpb-head .kpb-btn-line2{font-size:15px}#kpb-close{right:6px}#kpb-close .kpb-ico{width:22px;height:22px}#kpb-btn .kpb-btn-line2{font-size:10.5px}#kpb-btn{bottom:40px !important}}',
    ].join('');
    ...
    root.innerHTML = [
      ...
      headAssistLabel(resolveMascotUrl('')),
    ...
    btn.innerHTML = isMobileUi() ? mobileFabBtnLabel(resolveMascotUrl('')) : BTN_LABEL;
    ...
    if (isMobileUi()) {
      ...
      mascot.addEventListener('load', function () {
        alignMascotOverAiLabel();
      });
      fabStack.appendChild(mascot);
      fabStack.appendChild(btn);
      document.body.appendChild(fabStack);
    }

    /** Мобила: только кнопка в body; десктоп: стек с маскотом. */
    function fabShell() {
      return isMobileUi() ? btn : fabStack;
    }

    if (autoIntroActive) {
      fabShell().classList.add('kpb-hidden');
      ...
      fabStack.classList.remove('kpb-scroll-hidden');
    }
    ...
    function addBotReplyRevealed(text, onComplete, opts) {
      opts = opts || {};
      var onRevealStart = opts.onRevealStart;
      var full = String(text);
      var lines = full.split('\n');
      var el = document.createElement('div');
      el.className = 'kpb-msg bot';
      var lineIdx = 0;
      var started = false;
      ...
      function tick() {
        ...
        scrollMsgsToBottom();
        setTimeout(tick, REVEAL_LINE_MS);
      }
      tick();
    }
    ...
    function alignMascotOverAiLabel() {
      if (!mascot || mascot.style.display === 'none' || !isDesktopUi()) {
        if (mascot) mascot.style.marginLeft = '';
        return;
      }
      var target = btn.querySelector('.kpb-btn-line1') || btn.querySelector('.kpb-btn-copy');
      if (!target || !mascot.offsetWidth) return;
      var stackRect = fabStack.getBoundingClientRect();
      var tRect = target.getBoundingClientRect();
      var centerX = tRect.left + tRect.width / 2;
      mascot.style.marginLeft =
        Math.round(centerX - stackRect.left - mascot.offsetWidth / 2) + 'px';
    }

    function updateFabScrollVisibility() {
      if (open) {
        if (isMobileUi()) btn.classList.remove('kpb-scroll-hidden');
        else fabStack.classList.remove('kpb-scroll-hidden');
        return;
      }
      if (autoIntroActive) return;
      if (isMobileUi()) {
        if (pageScrollY() < MOBILE_FAB_SCROLL_PX) {
          btn.classList.add('kpb-scroll-hidden');
        } else {
          btn.classList.remove('kpb-scroll-hidden');
        }
    ...
    function pinFab() {
      ...
      var stackEl = fabStack;
      var h = stackEl.offsetHeight || FAB_HEIGHT_PX;
      var mobile = isMobileUi();
      if (mobile) {
        var side = 20;
        var bottom = MOBILE_FAB_BOTTOM_PX;
        var w = FAB_WIDTH_PX;
        var hBtn = btn.offsetHeight || FAB_HEIGHT_PX;
        ...
        var topMob = vv.offsetTop + vv.height - bottom - hBtn;
        btn.style.top = Math.max(vv.offsetTop, topMob) + 'px';
        return;
      }
      ...
      stackEl.style.width = '';
      if (!vv) {
        stackEl.style.left = 'auto';
        stackEl.style.right = FAB_RIGHT_PX + 'px';
        stackEl.style.top = 'auto';
        stackEl.style.bottom = FAB_BOTTOM_PX + 'px';
        alignMascotOverAiLabel();
        return;
      }
      stackEl.style.right = 'auto';
      stackEl.style.left =
        vv.offsetLeft + vv.width - FAB_RIGHT_PX - (stackEl.offsetWidth || w) + 'px';
      stackEl.style.bottom = 'auto';
      var topDesk = vv.offsetTop + vv.height - FAB_BOTTOM_PX - h;
      stackEl.style.top = Math.max(vv.offsetTop, topDesk) + 'px';
      alignMascotOverAiLabel();
    }
    ...
    function applyMascotUrl(url) {
      ...
      var fabMascot = btn.querySelector('.kpb-btn-mascot');
      if (fabMascot) fabMascot.src = activeMascotUrl;
      var headMascot = root.querySelector('.kpb-head-mascot');
      if (headMascot) headMascot.src = activeMascotUrl;
    ...
    function pulseFabAttention() {
      fabShell().classList.remove('kpb-pre-show', 'kpb-hidden');
      btn.classList.add('kpb-attention');
      fabShell().setAttribute('aria-hidden', 'false');
      setTimeout(function () {
    ...
    function collapsePanelToFab() {
      ...
      fabShell().classList.remove('kpb-hidden', 'kpb-scroll-hidden');
    ...
    function setOpen(v, opts) {
      ...
      fabShell().classList.toggle('kpb-hidden', open);
      fabShell().setAttribute('aria-hidden', open ? 'true' : 'false');
    ...
    function sendMessage(text) {
      ...
      fetch(API_BASE + '/api/chat', {
        ...
        .then(function (data) {
          ...
          addBotReplyRevealed(reply, function () {
            if (data.mapUrl) {
              var img = document.createElement('img');
