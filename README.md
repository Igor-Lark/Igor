...
- **Сохранение диалогов сайта.** Сейчас переписка с виджета **не пишется** на сервер (только в браузере клиента на время сессии). В MAX уходит фрагмент только если есть заявка/телефон. При необходимости — добавить лог диалогов на VPS (файл или БД).
