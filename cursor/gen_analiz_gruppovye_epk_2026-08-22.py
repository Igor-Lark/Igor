...
def build():
    ...
    add_bullets(
        ...
        [
            ...
            "Форматы 20–22: ТГО 99 показов / 1 клик; видео 7 / 0; карусель 4 / 0. Площадок в выгрузке нет (поле запроса пустое) — мусорные домены по имени не поймать.",
            "Аудитория РСЯ смещена: мужчины, 35+ и 55+, Android. Ядро Поиска — смартфоны 25–44.",
        ],
    )
    add_heading(doc, "3.3. Вывод по РСЯ", 2)
    add_p(
        doc,
        "Пользы нет, вреда по деньгам нет. Паузить по-прежнему не обязательно: вы сами оставили наблюдение, и сети уже сжимаются сами. "
        "Если к вс 23.08 / следующей Пн–Вс снова 0 конв. при 100+ показах — тогда пауза. Если всплывёт конверсия с мусорной площадки — в исключения, не выключать всю РСЯ сразу.",
        bold=True,
    )
    ...
    add_bullets(
        doc,
        [
            "РСЯ — не трогать, смотрим до конца недели Пн–Вс (23.08) и ещё 3–4 дня.",
            "Минусы: аренда, снять, неватрип. Не минусовать «выход в море» и «водные прогулки».",
            "Стратегию, бюджет 7 000, цены целей не менять. Кампания обучена, объём после минусов сам сел — это ожидаемо.",
            "Следующий срез: полный 17–23.08 или 23–30.08 + скрин обучения/цены конверсии. Для РСЯ — если можно, выгрузка по площадкам.",
        ],
    )
