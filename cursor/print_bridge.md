...
Живой клон на ПК сейчас: **`D:\CURSOR\print-bridge-git`** (watcher стартовал оттуда).  
Рядом может быть **`D:\CURSOR\print-bridge`** без рабочего git — в неё файл сам не попадёт.  
Очередь: `inbox/` **в обоих** клонах. Watcher смотрит обе папки.
