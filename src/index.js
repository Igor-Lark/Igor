...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      reply:
        'Извините, сервис временно недоступен. Капитан Олег: +7 917 675 0555, Наталья: +7 918 304-40-00',
    });
  }
});
...
app.get('/api/widget-config', (_req, res) => {
  res.json({
    name: config.botName,
    greeting:
      'Здравствуйте! Помогу выбрать катер или яхту.\nКапитан часто в море — связь может быть слабой. Наталья: +7 918 304-40-00. Спрашивайте — или оставите контакт, свяжемся )',
  });
});
